...
const BUDGET_MONTHS = [
  ...
  { m:'Dec', app:95,  act:78  },
  { m:'Jan', app:95,  act:72  },
  { m:'Feb', app:100, act:80  },
  { m:'Mar', app:100, act:85  },
  { m:'Apr', app:110, act:75  },
  { m:'May', app:110, act:68  },
  { m:'Jun', app:115, act:72  },
]

const REVENUE_MONTHS = [
  { m:'Jul', target:45, actual:38  },
  { m:'Aug', target:45, actual:48  },
  { m:'Sep', target:45, actual:52  },
  { m:'Oct', target:48, actual:56  },
  { m:'Nov', target:48, actual:59  },
  { m:'Dec', target:52, actual:65  },
  { m:'Jan', target:52, actual:68  },
  { m:'Feb', target:55, actual:74  },
  { m:'Mar', target:55, actual:80  },
  { m:'Apr', target:60, actual:83  },
  { m:'May', target:60, actual:87  },
  { m:'Jun', target:65, actual:92  },
]

const ACTIVITY_EVENTS = [
  { text: 'MOF: Budget execution report anchored — #FA event',  color: COLORS.operational },
  { text: 'AID TRACKER: Grant milestone verified — WST 2.4M',   color: COLORS.fiscal },
  ...
  { text: 'CBS → MOF: RTGS settlement confirmed',               color: COLORS.operational },
...
const REVENUE_PILLS = [
  { label: 'VAGST',    val: 'WST 142M' },
  { label: 'Customs',  val: 'WST 89M'  },
  { label: 'Other tax',val: 'WST 81M'  },
  { label: 'Non-tax',  val: 'WST 42M'  },
  { label: 'Grants',   val: 'WST 244M' },
]

function sevColor(sev) {
  return { crit: COLORS.critical, warn: COLORS.warning, info: COLORS.info, ok: COLORS.operational }[sev] || COLORS.textMuted
}
function sevBg(sev) {
  return { crit: COLORS.criticalBg, warn: COLORS.warningBg, info: COLORS.infoBg, ok: COLORS.operationalBg }[sev] || 'transparent'
}
function sevBorder(sev) {
  return { crit: COLORS.criticalBorder, warn: COLORS.warningBorder, info: COLORS.infoBorder, ok: COLORS.operationalBorder }[sev] || COLORS.border
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <KPICard label="Debt / GDP"          value="47.8%"       sub="DSA: Sustainable"          color={COLORS.fiscal}      icon="◎" />
      </div>

      {/* KPI Row 2 */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        <KPICard label="Aid Disbursed"       value="WST 89M"     sub="14 active projects"        color={COLORS.info}        icon="◈" />
        <KPICard label="Procurement"         value="23 contracts" sub="WST 156M OCDS tracked"    color={COLORS.text}        icon="◆" />
      ...
      {/* Three-column: Chart | Feed | Alerts */}
      <div style={{ display: 'grid', gridTemplateColumns: '40% 32% 1fr', gap: 16 }}>
        ...
        <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '18px 20px' }}>
          <SectionHeader
            title="Budget Execution FY 2025/26"
            subtitle="WST Millions · Appropriated vs Actual"
          />
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={BUDGET_MONTHS} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
              ...
              <Bar dataKey="app" name="Appropriated" fill={COLORS.govBlue}  radius={[2,2,0,0]} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {ALERTS.map((a, i) => (
              <div key={i} style={{
                ...
                border:        `1px solid ${sevBorder(a.sev)}`,
      ...
      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '18px 20px' }}>
        <SectionHeader
          title="Revenue Collection YTD — Cumulative"
          subtitle="WST Millions · Target vs Actual"
        />
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
          {REVENUE_PILLS.map(p => (
            <div key={p.label} style={{
              ...
              <span style={{ color: COLORS.text }}>{p.val}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
          {FISCAL_CALENDAR.map((ev, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontFamily: TYPOGRAPHY.mono, fontSize: 11, color: ev.c, marginBottom: 6, fontWeight: 700 }}>
                {ev.m}
              </div>
              <div style={{ fontFamily: TYPOGRAPHY.sans, fontSize: 12, color: COLORS.textMuted, lineHeight: 1.4 }}>
                {ev.ev}
              </div>
            </div>
          ))}
      ...
      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '18px 20px' }}>
        <SectionHeader
          title="One Government Interoperability Status"
          subtitle="InteroperabilityHub · Real-time fiscal data flows"
        />
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
          {INTEROP.map((io, i) => (
            <div key={i} style={{
              background:    io.ok ? COLORS.operationalBg : COLORS.blockedBg,
              border:        `1px solid ${io.ok ? COLORS.operationalBorder : COLORS.blockedBorder}`,
              borderRadius:  6,
              padding:       '10px 14px',
              display:       'flex',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                {io.ok && <div style={{ width: 6, height: 6, borderRadius: '50%', background: COLORS.operational, animation: 'pulse 2s infinite' }} />}
                <span style={{
                  fontFamily:    TYPOGRAPHY.mono,
                  fontSize:      11,
                  fontWeight:    700,
                  color:         io.ok ? COLORS.operational : COLORS.blocked,
                  letterSpacing: '0.5px',
                }}>
                  {io.status}
                </span>
